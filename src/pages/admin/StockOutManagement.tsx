...
import { CheckCircle, XCircle, Clock, Package, ShoppingCart } from 'lucide-react';
...
const StockOutManagement: React.FC = () => {
  ...
  const getStatusLabel = (status: StockOutStatus) => {
    switch (status) {
      case 'from_sales_order':
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Stock Out Management" 
        description="Manage outgoing stock requests from sales orders and manual requests"
      />
      ...
      <div className="flex items-center space-x-4">
        <Select value={statusFilter} onValueChange={(value: 'all' | StockOutStatus) => setStatusFilter(value)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="from_sales_order">From Sales Orders</SelectItem>
            ...
            <CardContent className="pt-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400">
                  {statusFilter !== 'all'
                    ? `No ${getStatusLabel(statusFilter as StockOutStatus).toLowerCase()} requests at the moment`
            ...
            <Card key={request.id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg flex items-center gap-2">
                    {request.sales_order_id && <ShoppingCart className="h-5 w-5" />}
                    {request.sales_orders?.sales_order_number || `Request to ${request.destination}`}
                  </CardTitle>
              ...
              <CardContent>
                <div className="space-y-4">
                    ...
                    {request.customer_name ? (
                      <div>
                        ...
                        <p className="font-medium">
                          {request.customer_name} ({request.customer_company})
                        </p>
                  ...
                  {(request.status === 'pending' || request.status === 'from_sales_order') && (
