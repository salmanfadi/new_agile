...
// Utility function to generate barcode canvas
const getBarcodeCanvas = (barcode: string): Promise<HTMLCanvasElement> => {
  return new Promise((resolve) => {
    const canvas = document.createElement('canvas');
    const barcodeGenerator = new BarcodeGenerator({ value: barcode });
    // @ts-ignore - The component has a getCanvas method
    const barcodeCanvas = barcodeGenerator.getCanvas();
    if (barcodeCanvas) {
...
const DedicatedBatchStockInPage: React.FC = () => {
  ...
  const processStockInMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['stockIn', 'batches'] });
      toast({
        title: 'Success',
        description: 'Stock in processed successfully',
        variant: 'default',
      });
      navigate('/warehouse/stock-in');
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.message || 'Failed to process stock in',
        variant: 'destructive',
      });
    },
  });
  ...
  const { data: stockInData, isLoading: isStockInLoading, error: stockInError } = useQuery({
    ...
    enabled: !!stockInId, // Only run the query when stockInId is available
  });
  ...
  // Process stock-in
  const { mutate: processStockIn, isPending } = useMutation({
    mutationFn: async ({ requestId, processedBy }: { requestId: string; processedBy: string }) => {
      const { data, error } = await supabase
        .from('stock_in')
        .update({ 
          status: 'processing',
