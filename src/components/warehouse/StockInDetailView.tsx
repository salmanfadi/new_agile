  ...
  return (
    <div className="border p-4 rounded-md mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <p><strong>Product:</strong> {stockIn.product?.name || 'Unknown Product'}</p>
          <p><strong>SKU:</strong> {stockIn.product?.sku || 'N/A'}</p>
          <p><strong>Category:</strong> {stockIn.product?.category || 'N/A'}</p>
        </div>
        <div>
          <p><strong>Boxes:</strong> {stockIn.boxes || 0}</p>
          <p><strong>Status:</strong> {stockIn.status}</p>
          <p><strong>Source:</strong> {stockIn.source || 'N/A'}</p>
          <p><strong>Submitted by:</strong> {stockIn.submitter?.name || 'Unknown'} ({stockIn.submitter?.username || 'N/A'})</p>
        </div>
      </div>
      <div>
