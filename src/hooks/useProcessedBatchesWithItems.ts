      ...
      try {
        ...
        const batches: ProcessedBatchWithItems[] = (data || []).map(batch => {
          ...
          if (batch.profiles && typeof batch.profiles === 'object' && batch.profiles !== null) {
            // Fixed TypeScript error by ensuring profiles.name exists before accessing it
            processorName = batch.profiles.name !== null && batch.profiles.name !== undefined ? batch.profiles.name : 'Unknown User';
          }
